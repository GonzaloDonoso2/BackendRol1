...
class FuncionesUsuario extends ConectorBaseDatos {
    ...
    function registrarUsuario ($nuevoValor) {
        ...
        $this->consultarBaseDatos($consulta);
        $consulta = "insert into PersonajesUsuarios (estado, idPersonaje, idUsuario) values ('ADQUIRIDO Y VIGENTE', 6, $idUsuario)";        
        $this->consultarBaseDatos($consulta);
    ...
    function iniciarSesion2 ($nuevoValor) {        
       
        $correoElectronico = $nuevoValor->correoElectronico;
        $contrasena = $nuevoValor->contrasena;
        $consulta = "select id, nombre from Usuarios where correoElectronico = '$correoElectronico' and contrasena = '$contrasena' and estado = 'VIGENTE' and idPerfil = 1";
        $respuesta = $this->consultarBaseDatos($consulta);
    ...
    function obtenerUsuarios2() {
        ...
        $consulta = "select id, nombre, estado from Usuarios where idPerfil = 2";
        $respuesta = $this->consultarBaseDatos($consulta);
    ...
    function habilitarUsuario ($nuevoValor) {  
        
        $id = $nuevoValor->id;  
        $consulta = "update Usuarios set estado = 'VIGENTE' where id = $id";        
        $this->consultarBaseDatos($consulta);        
        echo "El usuario fue habilitado de manera exitosa.";
    }
    
    function deshabilitarUsuario ($nuevoValor) {  
        
        $id = $nuevoValor->id;  
        $consulta = "update Usuarios set estado = 'NO VIGENTE' where id = $id";        
        $this->consultarBaseDatos($consulta);        
        echo "El usuario fue deshabilitado de manera exitosa.";
    }
    
    function buscarUsuario($nuevoValor) {
        
        $nombre = $nuevoValor->nombre;  
        $consulta = "select id, estado from Usuarios where nombre = '$nombre' and idPerfil = 2";
        $respuesta = $this->consultarBaseDatos($consulta);
