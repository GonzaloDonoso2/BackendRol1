...
class FuncionesBatalla extends ConectorBaseDatos {
    ...
    function registrarAtaque($nuevoValor) {
        ...
        $consulta = "select ataque, dano, nombre from BatallasEstadisticas where idBatalla = $idBatalla and idPersonaje = $idPersonaje1 and idUsuario = $idUsuario1";
        $respuesta = $this->consultarBaseDatos($consulta);
        ...
        if ($nombre === "MEDICO PARFALES") {
            
            $consulta = "select salud from BatallasEstadisticas where idBatalla = $idBatalla and idPersonaje = $idPersonaje2 and idUsuario = $idUsuario2";
            $respuesta = $this->consultarBaseDatos($consulta);
            ...
            $sanacion = random_int(1, 3);
            $nuevaSalud = ($salud + $sanacion);
            ...
            $consulta = "insert into Ataques (ataque, defensa, danoArmadura, danoPersonaje, estado, posicion, resultado, idPersonaje1, idPersonaje2, idTurno, idUsuario1, idUsuario2) values ($sanacion, 0, 0, 0, 'VIGENTE', '$posicion', 'ATAQUE EXISTOSO.', $idPersonaje1, $idPersonaje2, $idTurno, $idUsuario1, $idUsuario2)";
            $this->consultarBaseDatos($consulta);
    ...
    function obtenerMejoresResultadosBatallas() {
        ...
        $consultaExterior = "select id, nombre from Usuarios where idPerfil = 2";
        $respuestaExterior = $this->consultarBaseDatos($consultaExterior);
        $jsonRespuesta = array();

        foreach ($respuestaExterior as $key => $valueExterior) {

            $idUsuario = $valueExterior["id"];
            $nombre = $valueExterior["nombre"];
            $consultaInterior = "select count(resultado) as 'victorias' from BatallasResultados where idusuario = $idUsuario and resultado = 'VICTORIA'";
            $respuestaInterior = $this->consultarBaseDatos($consultaInterior);

            foreach ($respuestaInterior as $key => $valueInterior) {

                $victorias = $valueInterior["victorias"];             
                $jsonRespuesta[] = array(
                    "idUsuario" => $idUsuario,
